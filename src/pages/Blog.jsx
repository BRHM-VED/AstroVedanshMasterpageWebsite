import { Link, useSearchParams } from 'react-router-dom'
import { useSEO } from '../lib/seo.js'
import { PageHero } from '../components/Section.jsx'
import Pagination, { usePage } from '../components/Pagination.jsx'
import { POSTS, coverFor } from '../data/content.js'
import { useApi } from '../lib/api.js'

const PER_PAGE = 6

export default function Blog() {
  const { data: posts } = useApi('/posts', POSTS)
  const [searchParams] = useSearchParams()
  const totalPages = Math.max(1, Math.ceil(posts.length / PER_PAGE))
  const page = usePage(searchParams, totalPages)
  const shown = posts.slice((page - 1) * PER_PAGE, page * PER_PAGE)

  useSEO({
    title:
      page > 1
        ? `Astrology Blog — Page ${page} of ${totalPages}`
        : 'Astrology Blog — Kundli, Numerology, Panchang & Remedies',
    description:
      'Plain-language guides on Vedic astrology, numerology, sade sati, gemstones, kundli matching and panchang — no superstition, no scare tactics.',
    path: page > 1 ? `/blog?page=${page}` : '/blog',
  })

  return (
    <>
      <PageHero
        eyebrow="Simplifying Astrology for Everyone"
        title="The AstroVedansh Blog"
        sub="Plain-language guides on astrology, numerology, panchang and remedies — no superstition, no scare tactics."
      />
      <section className="section-pad">
        <div className="container-av">
          <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
            {shown.map((p) => (
              <Link key={p.slug} to={`/blog/${p.slug}`} className="card flex flex-col !p-0">
                <img src={coverFor(p)} alt="" loading="lazy" className="aspect-[1200/630] w-full object-cover" />
                <div className="flex flex-1 flex-col p-6">
                  <p className="font-heading text-xs font-bold uppercase tracking-wider text-gold-600">{p.category}</p>
                  <h2 className="mt-2 font-heading text-lg font-bold leading-snug text-maroon-900">{p.title}</h2>
                  <p className="mt-2 flex-1 text-sm text-maroon-950/70">{p.excerpt}</p>
                  <div className="mt-4 flex items-center justify-between">
                    <span className="text-xs text-maroon-950/50">
                      {new Date(p.date || p.published_at).toLocaleDateString('en-IN', { day: 'numeric', month: 'short', year: 'numeric' })}
                    </span>
                    <span className="font-heading text-sm font-bold text-maroon-700">Read →</span>
                  </div>
                </div>
              </Link>
            ))}
          </div>
          <Pagination page={page} totalPages={totalPages} basePath="/blog" />
        </div>
      </section>
    </>
  )
}
